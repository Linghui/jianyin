...
function share_to(m) {
	...
	var content = $('#t2').val();
	var shareUrl = $('#href').html();
	
	if (m == "baidu") {
		window
				.open('http://cang.baidu.com/do/add?it='
						+ encodeURIComponent(content)
						+ '&iu=' + encodeURIComponent(shareUrl)
						+ '&fr=ien#nw=1', 'baidu',
		...
		window
				.open(
						'http://shuqian.qq.com/post?from=3&title='
								+ encodeURIComponent(content) + '&uri='
		...
		void ((function(s, d, e) {
			...
			var f = 'http://v.t.sina.com.cn/share/share.php?', u = shareUrl, p = [
					'url=', e(u), '&title=', e(content), '&appkey=330242870' ]
					.join('');
			function a() {
				if (!window
						.open(
...
//						+ (screen.height - 500) / 2);
		...
		window.open("http://share.jianghu.taobao.com/share/addShare.htm?url="
				+ encodeURIComponent(shareUrl), 'taojianghu',
				'toolbar=0,status=0,width=550,height=400,left='
						+ (screen.width - 550) / 2 + ',top='
						+ (screen.height - 500) / 2);
	} else if (m == "gmail") {
		...
		var w = window.open(
				"https://mail.google.com/mail/?view=cm&fs=1&tf=1&ui=2&shva=1&to&su="
						+ c(content) + "&body=" + c(shareUrl), 'gmail',
		...
		try {
			var w = window.open(
					"http://share.51.com/share/share.php?type=8&title="
							+ encodeURIComponent(content) + '&vaddr='
							+ encodeURIComponent(shareUrl), '51',
					'toolbar=0,status=0,width=780,height=700,left='
							+ (screen.width - 780) / 2 + ',top='
							+ (screen.height - 700) / 2);
			window.opener.focus();
		...
		void ((function(s, d, e) {
			var f = 'http://t.sohu.com/third/post.jsp?link=', u = shareUrl;
			function a() {
				if (!window
						.open(
								[ f, e(u) ].join(''),
		...
		void ((function(s, d, e) {
			var f = 'http://bai.sohu.com/share/blank/addbutton.do?link=', u = shareUrl, l = content, p = [
					e(u), '&title=', e(l) ].join('');
			function a() {
				if (!window
						.open(
		...
		window.open(
				"http://sns.qzone.qq.com/cgi-bin/qzshare/cgi_qzshare_onekey?url="
						+ encodeURIComponent(shareUrl), 'qzone',
		...
		window
				.open(
						"https://skydrive.live.com/sharefavorite.aspx/.SharedFavorites/?marklet=1&mkt=zh-CN&top=1&url="
								+ encodeURIComponent(shareUrl)
								+ "&title="
								+ encodeURIComponent(content), 'live',
		...
		var itieba_share = 'http://tieba.baidu.com/i/sys/share?link='
				+ encodeURIComponent(shareUrl) + '&type='
		...
		window.open("http://mail.live.com/secure/start?action=compose&subject="
				...
				+ encodeURIComponent(shareUrl), 'hotmail',
		...
		(function() {
			f = 'http://delicious.com/save?url='
					+ encodeURIComponent(shareUrl) + '&title='
					+ encodeURIComponent(content) + '&v=5&';
			a = function() {
				if (!window
		...
		window.open("http://digg.com/submit?type=0&url="
				+ encodeURIComponent(shareUrl), 'digg',
		...
		window
				.open(
						'http://apps.hi.baidu.com/share/?url=' + encodeURIComponent(shareUrl)+ '&title='
						+ encodeURIComponent(content),
						'baiduhi',
		...
		window.open('http://www.139.com/share/share.php?title='
				...
				+ encodeURIComponent(shareUrl), 't139',
		...
		window.open("http://compose.mail.yahoo.com/?subject="
				+ encodeURIComponent(shareUrl) + "&body="
				+ encodeURIComponent(content), 'ymail',
		...
		var t = d.selection ? (d.selection.type != 'None' ? d.selection
				...
				: '');
		void (saveit = window
				.open('http://wz.csdn.net/storeit.aspx?t=' + escape(content)
						+ '&u=' + escape(shareUrl) + '&c=' + escape(t),
						'csdn',
